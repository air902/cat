 ...
 * 驳回和撤销选手的转会申请
 */
@WebServlet("/manager/AdminTransferBanServlet")
public class AdminTransferBanServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int result = transferInFoService.transferBan(transferPlayerId);
        if(result>0){
            HttpSession session = request.getSession();
            session.setAttribute("classify",searchClassify);
            session.setAttribute("playerName",searchName);
            session.setAttribute("cp",cp);
            response.sendRedirect("/transfer_war_exploded/manager/AdminDoTransferFindServlet");
        }else{
